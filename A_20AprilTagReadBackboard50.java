...
@Autonomous(name="2+0|AprilTagRedBackboard", group="Worlds_Read")

public class A_20AprilTagReadBackboard50 extends LinearOpMode {
        ...
        gain.setGain(90);
            ...
            if (OD.getIntLocation() == 3) { // If the location is 3 then do ...
                ...
                Actions.runBlocking(new SequentialAction(
                        ...
                        drive.MoveXYHACEL(-43, 0, -100,-20, 15),//-20,50(10)
...
while(whileTrue < 1000){
                if (tagProcessor.getDetections().size() > 0) { // if it sees something
                    ...
                    if (tag.id == (6)) {
                        ...
                        double x = Math.abs(4 * tag.ftcPose.x);
                        double y = Math.abs(-2.2 * tag.ftcPose.y);
                        telemetry.speak(String.valueOf(tag.id));
                        telemetry.addData("Tag: ",tag.id);
                        telemetry.update();

                        Actions.runBlocking(new SequentialAction(drive.MoveXYACEL((x-28), (y ), -20, 20)));
                        whileTrue = 1001;
                        ...
                        Actions.runBlocking(new SequentialAction(drive.MoveXYACEL((x-24), (y ), -20, 20)));
                        whileTrue = 1001;
                    }else{}
                ...
                Actions.runBlocking(new SequentialAction(
                        ...
                        //Y+ robot goes towards the backboard
                        //drive.waitS(waitTime),
                        drive.MoveXYACEL(-53, 9, -20, 20),//-20,50(10)
                ...
                while(whileTrue < 1000){
                    if (tagProcessor.getDetections().size() > 0) { // if it sees something
                        ...
                        if (tag.id == (6)) {
                            ...
                            double x = Math.abs(4 * tag.ftcPose.x);
                            double y = Math.abs(-1.8 * tag.ftcPose.y);
                            telemetry.speak(String.valueOf(tag.id));
                            telemetry.addData("Tag: ",tag.id);
                            telemetry.update();

                            Actions.runBlocking(new SequentialAction(drive.MoveXYACEL((x-13), (y ), -20, 20)));
                            whileTrue = 1001;
                        }else{}
            ...
            ///dwgfuhufghowfhvpqeufhg qpwfuhpiuwp frrouqpror rpooroeouqpwr 2
                ...
                Actions.runBlocking(new SequentialAction(
                        ...
                        //Y+ robot goes towards the backboard
                        //drive.waitS(waitTime),
                        drive.MoveXYACEL(-46, 17.1, -20, 20),//-20,50(10)
                ...
                while(whileTrue < 1000){
                    if (tagProcessor.getDetections().size() > 0) { // if it sees something
                        ...
                        if (tag.id == (6)) {
                            ...
                            double x = Math.abs(4 * tag.ftcPose.x);
                            double y = Math.abs(-2.6 * tag.ftcPose.y);
                            telemetry.speak(String.valueOf(tag.id));
                            telemetry.addData("Tag: ",tag.id);
                            telemetry.update();
                            ...
                            double x = Math.abs(4 * tag.ftcPose.x);
                            double y = Math.abs(-2.5 * tag.ftcPose.y);
                            telemetry.speak(String.valueOf(tag.id));
                            telemetry.addData("Tag: ",tag.id);
                            telemetry.update();

                            Actions.runBlocking(new SequentialAction(drive.MoveXYACEL((x+14), (y -1.5), -20, 20)));
                            whileTrue = 1001;
                        }else{}
